...
//
// Exception: base positions 31/32/33 (`'` `.` `,`) are mod-tap homes (RCTL_T/RGUI_T/
// RALT_T on plain KC_QUOT/KC_DOT/KC_COMM), so their `"`/`!`/`?` shift can't ride a key
// override — a mod-tap never taps a custom keycode. SYM_MODTAP_SHIFT (below) supplies it
// from process_record_user, reading the same SY_*_SHIFTED the table does.
